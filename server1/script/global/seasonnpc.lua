...
function main()
	...
	if (GetLevel() < 90) then
		Talk(1, "", "§¼ng cÊp nh©n vËt trªn 90 cÊp míi cã thÓ lµm nhiÖm vô D· TÈu")
	return
	end
...
if (tl_gettaskcourse() == 2) then
	...
	if (tl_gettaskcourse() == 3) then
		...
		Say("<color=green>D· TÈu:<color><enter>VŞ "..GetPlayerSex().." nµy ®· hoµn thµnh tÊt c¶ ®­îc <color=yellow>"..myCountTimes.."<color> nhiÖm vô, ng­¬i cã muèn lµm n÷a kh«ng?",2,"§­¬ng nhiªn, mau cho ta biÕt nhiÖm vô tiÕp theo lµ g×/Task_TaskProcess","§Ó ta nghØ ng¬i mét l¸t ®·! Ta bËn råi/Task_Wait");
...
function Task_GiveAward()
	...
	local nTotalTaskNum = tl_counttasklinknum(1); -- »ñÈ¡µ±Ç°Íæ¼ÒÒ»¹²×öÁË¶àÉÙ´ÎÈÎÎñ
	if (nTotalTaskNum ~= 0 and mod(nTotalTaskNum, 2) == 0 and GetTask(TKS_TASKLINK_SPITEM) ~= nTotalTaskNum) then
		SetTask(TKS_TASKLINK_SPITEM, nTotalTaskNum);
		local tbItem = {tbProp = {6, 1, 2374, 1, 0, 0}}
		tbAwardTemplet:GiveAwardByList(tbItem, "seasonnpc_2task")
		Msg2Player(format("Chóc mõng ®¹i hiÖp ®· hoµn thµnh liªn tiÕp %d nhiÖm vô D· TÈu, nhËn ®­îc phÇn th­ëng %s!", 2, "B¶o r­¬ng Tèng Kim cña D· TÈu"));
	end
	...
	-- Sap xep lai de tien luon hien dau tien
	local nMoneyIndex = 0
	for i=1,3 do
	...
	-- Neu co tien va khong o vi tri dau tien, hoan doi
