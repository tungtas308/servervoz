...
local nVoLamLenh = CalcEquiproomItemCount(6,1,4903,-1);
	Describe("Sè l­îng Vâ L©m lÖnh hiÖn cã: <color=yellow>: "..nVoLamLenh.."<color><enter>     Tû lÖ ®æi 50 Vâ l©m lÖnh nhËn ®­îc ngÉu nhiªn MÆt N¹<enter>",11,
	"Ta ®ång ý/tadongymatna",
	"Ta sÏ quay l¹i sau!/no"
	);
end

function tadongymatna()
local nVoLamLenh = CalcEquiproomItemCount(6,1,4903,-1);
	if nVoLamLenh > 50 then
		local nChance = random(1, 100);
		if nChance <= 70 then
		ItemIndex = AddItem(0,11,584,1,0,0)
		ITEM_SetExpiredTime(ItemIndex, 10080);
		SetItemBindState(ItemIndex, -2)
		SyncItem(ItemIndex); 
		ConsumeEquiproomItem(50,6,1,4903,-1)
		else
			ItemIndex = AddItem(0,11,585,1,0,0)
			ITEM_SetExpiredTime(ItemIndex, 10080);
			SetItemBindState(ItemIndex, -2)
			SyncItem(ItemIndex); 
			ConsumeEquiproomItem(50,6,1,4903,-1)
...
function RuongHKMP()
local nVoLamLenh = CalcEquiproomItemCount(6,1,4903,-1);
	Describe("Sè l­îng Vâ l©m lÖnh hiÖn cã: <color=yellow>: "..nVoLamLenh.."<color><enter>     Tû lÖ ®æi 50 Hoµng Kim LÖnh = 1 R­¬ng Hoµng Kim M«n Ph¸i <enter>",11,
...
local nHoangKimLenh = CalcEquiproomItemCount(4,2059,1,1);
	if nHoangKimLenh > 49 then
		ItemIndex = AddItem(6,1,4879,1,0,0)
		SyncItem(ItemIndex); 
		ConsumeEquiproomItem(50,4,2059,1,1)
	else
