...
function main()
	...
	if GetTask( TASKID_DAY_PHLT ) ~= tonumber(date("%m%d")) then
		SetTask( TASKID_COUNT_PHLT , 0)
		SetTask( TASKID_DAY_PHLT , tonumber(date("%m%d")))
	end
	if (GetTask( TASKID_COUNT_PHLT ) >= SoLanThamGiaPHLTTrongNgay) and (GetTask(TASKID_HOUR_PHLT) ~= tonumber(GetLocalDate("%m%d%H"))) then
		Say(CD_SIGNNPCNAME..": Ng­¬i ®· tham gia "..GetTask( TASKID_COUNT_PHLT ).."/"..SoLanThamGiaPHLTTrongNgay.." lÇn trong ngµy, h·y quay l¹i ngµy mai!", 0)
		return
	end
